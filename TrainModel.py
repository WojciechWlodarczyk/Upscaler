...
def train(model, path, part_of_data = 1, start_epoch = 0, best_test_loss = float('inf'), rerun = False):
    #batches = 8
    batches = 1
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        log_line = f"Epoch [{epoch + 1}/{num_epochs}] Lr: {new_lr:.7f} B: {batches} Train Loss: {epoch_loss:.15f} Test Loss: {test_loss:.15f}"
        ...
        if test_loss < best_test_loss:
            ...
            returning_count += 1
            log_line += f" => return to previous weights ({returning_count} attempt)"
            model.load_state_dict(best_model_state)
            optimizer.load_state_dict(best_optimizer_state)

        Log(log_line)

        if returning_count > 30:
